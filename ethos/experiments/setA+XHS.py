...
def nested_cross_val(pipe,parameters,X,y,name):
    ...
    scores2 = {}
    scores2.setdefault('fit_time', [])
    scores2.setdefault('score_time', [])
    scores2.setdefault('test_F1', [])
    scores2.setdefault('test_Precision', [])
    scores2.setdefault('test_Recall', [])
    scores2.setdefault('test_Accuracy', [])
    scores2.setdefault('test_Specificity', [])
    scores2.setdefault('test_Sensitivity', [])
    ...
    for train_index, test_index in splits:
        ...
        clf = GridSearchCV(estimator=pipe, param_grid=parameters, cv=inner_cv, n_jobs=18, verbose=1, scoring='f1')
        a = time.time()
        ...
        score_time = time.time() - a
        scores2['fit_time'].append(fit_time)
        scores2['score_time'].append(score_time)
        scores2['test_F1'].append(f1_score(y_t,y_preds,average='macro'))
        scores2['test_Precision'].append(precision_score(y_t,y_preds,average='macro'))
        scores2['test_Recall'].append(recall_score(y_t,y_preds,average='macro'))
        scores2['test_Accuracy'].append(accuracy_score(y_t,y_preds))
        scores2['test_Specificity'].append(specificity(y_t,y_preds))
        scores2['test_Sensitivity'].append(sensitivity(y_t,y_preds))

    #for k in scores:
    #    print(str(name)+" "+str(k)+": "+str(sum(scores[k])/10))

    #for k in scores:
    #    print(str(name)+" "+str(k)+": "+str(sum(scores2[k])/10))
    ...
    print ("{:<7} | {:<7} {:<7} {:<7} {:<7} {:<7} {:<7} {:<7} {:<7}".format(str(name)[:7],
        ...
        str('%.4f' % (sum(scores['test_Specificity'])/10)), str('%.4f' % (sum(scores['test_Sensitivity'])/10))))
    f.write("{:<7} | {:<7} {:<7} {:<7} {:<7} {:<7} {:<7} {:<7} {:<7} \n".format(str(name)[:7],
        str('%.4f' % (sum(scores2['fit_time'])/10)), str('%.4f' % (sum(scores2['score_time'])/10)), str('%.4f' % (sum(scores2['test_F1'])/10)),
        str('%.4f' % (sum(scores2['test_Precision'])/10)), str('%.4f' % (sum(scores2['test_Recall'])/10)), str('%.4f' % (sum(scores2['test_Accuracy'])/10)),
        str('%.4f' % (sum(scores2['test_Specificity'])/10)), str('%.4f' % (sum(scores2['test_Sensitivity'])/10))))
    f.close()
    print ("{:<7} | {:<7} {:<7} {:<7} {:<7} {:<7} {:<7} {:<7} {:<7}".format(str(name)[:7],
        str('%.4f' % (sum(scores2['fit_time'])/10)), str('%.4f' % (sum(scores2['score_time'])/10)), str('%.4f' % (sum(scores2['test_F1'])/10)),
        str('%.4f' % (sum(scores2['test_Precision'])/10)), str('%.4f' % (sum(scores2['test_Recall'])/10)), str('%.4f' % (sum(scores2['test_Accuracy'])/10)),
...
X_t, y_t = Preproccesor.load_external_data(True)

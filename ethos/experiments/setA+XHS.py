...
def nested_cross_val(pipe, parameters, X, y, name):
    ...
    for train_index, test_index in splits:
        ...
        scores2['test_Precision'].append(
            precision_score(y_t, y_preds, average='macro'))
        scores2['test_Recall'].append(
            recall_score(y_t, y_preds, average='macro'))
    ...
    print("{:<7} | {:<7} {:<7} {:<7} {:<7} {:<7} {:<7} {:<7} {:<7}".format(str(name)[:7],
                                                                           ...
                                                                           str('%.4f' % (sum(scores['test_Specificity'])/10)), str('%.4f' % (sum(scores['test_Sensitivity'])/10))))
    f.write("{:<7} | {:<7} {:<7} {:<7} {:<7} {:<7} {:<7} {:<7} {:<7} \n".format(str(name)[:7],
                                                                                str('%.4f' % (sum(scores2['fit_time'])/10)), str('%.4f' % (
                                                                                    sum(scores2['score_time'])/10)), str('%.4f' % (sum(scores2['test_F1'])/10)),
                                                                                str('%.4f' % (sum(scores2['test_Precision'])/10)), str('%.4f' % (sum(
                                                                                    scores2['test_Recall'])/10)), str('%.4f' % (sum(scores2['test_Accuracy'])/10)),
